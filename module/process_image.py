...
def process_image(img_file):
    ...
    if(h < height and w < width):
        ...
        if(h > height):
            ...
            new_img = cv2.copyMakeBorder(img, 0, 0, left, right, cv2.BORDER_CONSTANT, value=color)
        else:
            top, bottom = int(((height*w)/width)/2), int(((height*w)/width)/2)
            new_img = cv2.copyMakeBorder(img, top, bottom, 0, 0, cv2.BORDER_CONSTANT, value=color)
